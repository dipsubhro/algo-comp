...
functions = {'sphere': sphere, 'sum_of_squares': sum_of_squares, 'sum_of_different_powers': sum_of_different_powers, 'step': step, 'brown': brown, 'zakharov': zakharov, 'dixon_price': dixon_price, 'schumer_steiglitz': schumer_steiglitz, 'csendes': csendes, 'sixth_power': sixth_power, 'powell': powell, 'quartic': quartic, 'rotated_hyper_ellipsoid': rotated_hyper_ellipsoid, 'discus': discus, 'exponential': exponential}
bounds = {'sphere': (-5.12, 5.12), 'sum_of_squares': (-10, 10), 'sum_of_different_powers': (-1, 1), 'step': (-100, 100), 'brown': (-1, 4), 'zakharov': (-5, 10), 'dixon_price': (-10, 10), 'schumer_steiglitz': (-100, 100), 'csendes': (-1, 1), 'sixth_power': (-1, 1), 'powell': (-4, 5), 'quartic': (-1.28, 1.28), 'rotated_hyper_ellipsoid': (-65.536, 65.536), 'discus': (-100, 100), 'exponential': (-1, 1)}
...
for name, func in functions.items():
    ...
    print(f'{name:<15} {np.min(results):<12.6f} {np.mean(results):<12.6f} {np.median(results):<12.6f} {np.max(results):<12.6f} {np.std(results):<12.6f}')