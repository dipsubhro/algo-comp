...
def export_results(func_names, num_runs=30, formats='all'):
    ...
    for i, func_name in enumerate(func_names, 1):
        print(f'[{i}/{len(func_names)}] {func_name}...', end=' ')
        results, _ = run_multiple_experiments(func_name, num_runs)
        all_results[func_name] = {'results': results, 'stats': calculate_statistics(results)}
        print('✓')
    ...
    if formats in ['csv', 'all']:
        ...
        with open(filename, 'w', newline='') as f:
            ...
            for func_name, data in all_results.items():
                stats = data['stats']
                writer.writerow([func_name, f'{stats['min']:.8f}', f'{stats['mean']:.8f}', f'{stats['median']:.8f}', f'{stats['std']:.8f}', f'{stats['max']:.8f}'])
        print(f'✓ CSV: {filename}')
    if formats in ['json', 'all']:
        filename = f'ga_results_{timestamp}.json'
        export_data = {}
        for func_name, data in all_results.items():
            export_data[func_name] = {'results': [float(r) for r in data['results']], 'statistics': {k: float(v) for k, v in data['stats'].items()}}
    ...
    if formats in ['md', 'all']:
        ...
        with open(filename, 'w') as f:
            ...
            for func_name, data in all_results.items():
                stats = data['stats']
                f.write(f'| {func_name} | {stats['min']:.8f} | {stats['mean']:.8f} | {stats['median']:.8f} | {stats['std']:.8f} | {stats['max']:.8f} |\n')
