...
def show_single_result(func_name, results, time_taken):
    ...
    print('=' * 60)
    table_data = [['Runs', len(results)], ['Min', format_number(stats['min'])], ['Mean', format_number(stats['mean'])], ['Median', format_number(stats['median'])], ['Std Dev', format_number(stats['std'])], ['Max', format_number(stats['max'])], ['Time', f'{time_taken:.2f}s']]
...
def show_all_results(all_results, time_taken):
    ...
    for func_name, results in all_results.items():
        stats = calculate_statistics(results)
        results_data.append({'name': func_name, 'min': stats['min'], 'mean': stats['mean'], 'median': stats['median'], 'std': stats['std']})
    ...
    for i, data in enumerate(results_data, 1):
        ...
        table_data.append([rank, data['name'], format_number(data['min']), format_number(data['mean']), format_number(data['median']), format_number(data['std'])])
    print(tabulate(table_data, headers=['Rank', 'Function', 'Min', 'Mean', 'Median', 'Std Dev'], tablefmt='grid', disable_numparse=True))
    print(f'\n{'=' * 100}')
