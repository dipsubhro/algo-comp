...
def plot_individual_function(func_name, results, history, stats):
    ...
    ax2.axvline(stats['mean'], color=C_MEAN, linestyle='--', linewidth=2, label=f'Mean: {format_number(stats['mean'])}')
    ax2.axvline(stats['median'], color=C_MED, linestyle='--', linewidth=2, label=f'Median: {format_number(stats['median'])}')
    ...
    for bar, val in zip(bars, values):
        ax3.text(bar.get_width() + max_val * 0.02, bar.get_y() + bar.get_height() / 2, format_number(val), va='center', ha='left', fontsize=8.5, color='#ccc', fontweight='bold')
    ...
    for sp in ax3.spines.values():
        sp.set_color('#333')
    ax3.invert_yaxis()
    fig.suptitle(f'GA Results — {func_name}', fontsize=16, fontweight='bold', color='#eee', y=1.02)
...
def plot_all_functions_combined(all_results, plot_data):
    ...
    def _draw_hbar(ax, title, values, color):
        ...
        max_val = max((v for v in values if v != 0), default=1)
        for bar, val in zip(bars, values):
            ax.text(bar.get_width() + max_val * 0.015, bar.get_y() + bar.get_height() / 2, format_number(val), va='center', ha='left', fontsize=7.5, color='#ccc', fontweight='bold')
    ...
    for i, fname in enumerate(top5):
        if fname in plot_data:
            ax5.plot(plot_data[fname], label=fname, color=cmap[i], linewidth=2.5 if i == 0 else 1.8, alpha=0.9)
    ...
    for sp in ax5.spines.values():
        sp.set_color('#333')
    fig.suptitle('GA Benchmark — All 15 Functions', fontsize=20, fontweight='bold', color='#eee', y=0.995)
